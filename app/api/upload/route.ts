...
export async function POST(request: NextRequest) {
  ...
  const localFolderName = isVideo ? "vedio" : "uploads";
  const cloudinaryFolder = isVideo ? "shadow-commerce/vedio" : "shadow-commerce";
  ...
  const uploadDir = path.join(process.cwd(), "public", localFolderName);
  await mkdir(uploadDir, { recursive: true });
  await writeFile(path.join(uploadDir, safeName), buffer);

  return NextResponse.json({ url: `/${localFolderName}/${safeName}` });
}
